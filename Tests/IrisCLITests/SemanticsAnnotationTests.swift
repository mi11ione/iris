...
/// Validates the `--semantics` vocabulary.
...
struct SemanticsAnnotationTests {
    ...
    @Test func registerReadsAndWrites() {
        #expect(annotate(0x9100_0421) == "reads=x1 writes=x1")
        #expect(annotate(0xAA01_03E0) == "reads=x1 writes=x0")
    }

    @Test func stackPointerRendersAsSP() {
        #expect(annotate(0xD100_43FF) == "reads=sp writes=sp")
    }
    ...
    @Test func vectorRegistersUseVNames() {
        #expect(annotate(0x4E28_4820) == "reads=v0,v1 writes=v0")
    }

    @Test func memoryAccessVocabulary() {
        #expect(annotate(0xF940_0020).contains("mem=load"))
        #expect(annotate(0xF900_0020).contains("mem=store"))
        #expect(annotate(0xC85F_7C20).contains("mem=exclusive-load"))
        #expect(annotate(0xC800_7C41).contains("mem=exclusive-store"))
        #expect(annotate(0xF880_0020).contains("mem=prefetch"))
        #expect(annotate(0xC8E0_FC41).contains("mem=atomic"))
    }

    @Test func orderingVocabulary() {
        #expect(annotate(0xC8DF_FC20).contains("order=acquire"))
        #expect(annotate(0xC89F_FC20).contains("order=release"))
        #expect(annotate(0xC8E0_FC41).contains("order=acquire-release"))
        #expect(!annotate(0xF940_0020).contains("order="))
    }

    @Test func branchClassVocabulary() {
        #expect(annotate(0x1400_0001) == "branch=direct")
        #expect(annotate(0x9400_0001) == "writes=x30 branch=call")
        #expect(annotate(0xD61F_0000) == "reads=x0 branch=indirect")
        #expect(annotate(0xD65F_03C0) == "reads=x30 branch=return")
        #expect(annotate(0x5400_0040) == "flags=r:nzcv branch=conditional")
        #expect(annotate(0xD400_0001) == "branch=exception")
    }

    @Test func flagLetterVocabulary() {
        #expect(annotate(0xB100_0420) == "reads=x1 writes=x0 flags=w:nzcv")
        #expect(annotate(0x9A01_0020) == "reads=x1 writes=x0 flags=r:c")
        #expect(annotate(0xFA41_0800) == "reads=x0 flags=r:nzcv,w:nzcv")
    }
