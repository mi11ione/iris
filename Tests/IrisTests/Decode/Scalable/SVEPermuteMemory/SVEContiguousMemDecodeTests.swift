...
/// Validates the contiguous scalar-base LD1 loads at 0xA4/0xA5.
...
struct SVEContiguousLoadDecodeTests {
    private static let rows: [(UInt32, Mnemonic, String)] = [
        ...
        (0xA400_4000, .ld1b, "ld1b { z0.b }, p0/z, [x0, x0]"),
        (0xA460_4000, .ld1b, "ld1b { z0.d }, p0/z, [x0, x0]"),
    ...
    @Test func aLoadReadsTheBaseAndWritesTheSingleVector() {
        let d = decode(0x8402_5023)
        #expect(canonicalIndices(d.semanticReads) == [1, 34])
        #expect(canonicalIndices(d.semanticWrites) == [35])
        #expect(d.scalableReads.containsPredicate(4))
    }
...
/// Validates the first-fault (`LDFF1`) and non-fault (`LDNF1`) contiguous
/// loads.
...
struct SVEReplicateQuadDecodeTests {
    private static let rows: [(UInt32, Mnemonic, String)] = [
        ...
        (0xA401_2000, .ld1rqb, "ld1rqb { z0.b }, p0/z, [x0, #16]"),
        ...
        (0xA421_2000, .ld1rob, "ld1rob { z0.b }, p0/z, [x0, #32]"),
...
/// the SVE2p1 quadword LD2Q-4Q / ST2Q-4Q).
...
struct SVEStructuredMemDecodeTests {
    private static let rows: [(UInt32, Mnemonic, String)] = [
        ...
        (0xA421_E000, .ld2b, "ld2b { z0.b, z1.b }, p0/z, [x0, #2, mul vl]"),
    ...
    @Test func aStructuredLoadWritesTheWholeGroup() {
        let d = decode(0xA423_D022)
        #expect(text(0xA423_D022) == "ld2b { z2.b, z3.b }, p4/z, [x1, x3]")
        #expect(canonicalIndices(d.semanticWrites) == [34, 35])
        #expect(canonicalIndices(d.semanticReads) == [1, 3])
    }

    @Test func aStructuredStoreReadsTheWholeGroupAndWritesNothing() {
        let d = decode(0xE430_E000)
        #expect(canonicalIndices(d.semanticWrites) == [])
        #expect(canonicalIndices(d.semanticReads) == [0, 32, 33])
    }
}
...
struct SVEContiguousStoreDecodeTests {
    private static let rows: [(UInt32, Mnemonic, String)] = [
        ...
        (0xE500_E000, .st1w, "st1w { z0.q }, p0, [x0]"),
    ...
    @Test func aStoreReadsItsDataAndAddressAndWritesNothing() {
        let d = decode(0xE402_5023)
        #expect(text(0xE402_5023) == "st1b { z3.b }, p4, [x1, x2]")
        #expect(canonicalIndices(d.semanticReads) == [1, 2, 35])
        #expect(canonicalIndices(d.semanticWrites) == [])
    }
}

/// Validates the LDR/STR register fill/spill forms.
...
struct SVEFillSpillDecodeTests {
    ...
    @Test func fillWritesTheVectorAndSpillReadsIt() {
        let ldr = decode(0x8580_5461)
        #expect(text(0x8580_5461) == "ldr z1, [x3, #5, mul vl]")
        #expect(ldr.memoryAccess == .load)
        #expect(canonicalIndices(ldr.semanticReads) == [3])
        #expect(canonicalIndices(ldr.semanticWrites) == [33])

        let str = decode(0xE580_5461)
        #expect(str.memoryAccess == .store)
        #expect(canonicalIndices(str.semanticReads) == [3, 33])
        #expect(canonicalIndices(str.semanticWrites) == [])
    }

    @Test func thePredicateFillRidesTheScalableRegisterFile() {
        let ldr = decode(0x8580_0000)
        #expect(ldr.scalableWrites.containsPredicate(0))
        #expect(canonicalIndices(ldr.semanticWrites) == [])
        let str = decode(0xE580_0000)
        #expect(str.scalableReads.containsPredicate(0))
    }
}

/// Validates that reserved memory encodings.
