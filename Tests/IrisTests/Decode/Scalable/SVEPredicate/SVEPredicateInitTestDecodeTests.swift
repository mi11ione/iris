...
/// Validates PTRUE / PTRUES / PFALSE / PTEST — the predicate initialise-and-test
/// group. These are the instructions that manufacture a predicate out of thin
/// air (from a size and a count pattern) or fold one back into the condition
/// flags, so their contract is the pattern operand, the element size, the
/// result-predicate write, and the NZCV write that only the S-suffixed and
/// PTEST forms carry.
...
struct SVEPredicateInitTestDecodeTests {
    @Test func ptrueCarriesItsSizeAndPatternAndWritesOnlyItsDestination() {
        let d = decode(0x2518_E000) // ptrue p0.b, pow2
    ...
    @Test func ptrueDecodesEveryElementSize() {
        let cases: [(UInt32, ScalarSize, UInt8, UInt8)] = [
            (0x2518_E000, .b, 0, 0), // pow2
            (0x2558_E105, .h, 8, 5), // vl8
            (0x2598_E1E9, .s, 15, 9), // unnamed pattern 15
            (0x25D8_E3EF, .d, 31, 15), // all
        ]
    ...
    @Test func ptrueRejectsItsReservedBits() {
        #expect(decode(0x251A_E000).mnemonic == .undefined) // bits 18:17 nonzero
        #expect(decode(0x2518_E010).mnemonic == .undefined) // bit 4 nonzero
    }

    @Test func pfalseWritesAnAllFalseByteElementPredicate() {
        let d = decode(0x2518_E407) // pfalse p7.b
    ...
    @Test func pfalseRejectsEveryReservedField() {
        // PFALSE has no size, no governing predicate and no pattern: every one
        // of those fields is a fixed zero, and a nonzero is a different encoding.
        #expect(decode(0x2558_E400).mnemonic == .undefined) // size field
        #expect(decode(0x2519_E400).mnemonic == .undefined) // bits 18:16
        #expect(decode(0x2518_E600).mnemonic == .undefined) // bit 9
        #expect(decode(0x2518_E410).mnemonic == .undefined) // bits 8:4
    }

    @Test func ptestReadsBothPredicatesAndWritesOnlyTheFlags() {
        let d = decode(0x2550_C440) // ptest p1, p2.b
    ...
    @Test func ptestReachesTheHighestPredicateRegisters() {
        let d = decode(0x2550_FDC0) // ptest p15, p14.b
        #expect(predicates(d.scalableReads) == [14, 15])
    }

    @Test func ptestRejectsItsReservedBits() {
        #expect(decode(0x2510_C000).mnemonic == .undefined) // size field must be 01
        #expect(decode(0x2551_C000).mnemonic == .undefined) // bits 18:16
        #expect(decode(0x2550_C200).mnemonic == .undefined) // bit 9
        #expect(decode(0x2550_C001).mnemonic == .undefined) // bits 4:0
    }
}
