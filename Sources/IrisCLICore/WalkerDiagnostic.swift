...
public struct WalkerDiagnostic: Sendable, Hashable, CustomStringConvertible {
    ...
    public enum Kind: Sendable, Hashable {
        /// A `fat_arch` entry's byte range falls outside the file; the slice
        /// is excluded from selection.
        case fatSliceOutOfBounds
        /// A load command's declared `cmdsize` is too small (< 8) or runs past
        /// the load-command region; the walk stops at the previous command.
        case loadCommandInvalid
        /// The load-command region itself exceeds the slice; the walk covers
        /// the in-bounds prefix only.
        case loadCommandRegionTruncated
        /// More than one of a load command the walker expects once
        /// (`LC_SYMTAB`, `LC_FUNCTION_STARTS`, `LC_DATA_IN_CODE`); the first
        /// is used.
        case duplicateLoadCommand
        /// A segment's declared section count does not fit its `cmdsize`; the
        /// in-budget prefix is walked.
        case sectionTableTruncated
        /// A code section's file content runs past the slice; the section is
        /// clamped to the bytes that exist.
        case sectionContentOutOfBounds
        /// A code section declares zero bytes; it is listed nowhere.
        case sectionEmpty
        /// The `LC_DATA_IN_CODE` table region is outside the slice; the table
        /// is ignored.
        case dataInCodeRegionOutOfBounds
        /// The `LC_DATA_IN_CODE` region size is not a multiple of the 8-byte
        /// entry size; the floor prefix is decoded.
        case dataInCodeRegionTruncated
        /// A `data_in_code_entry` lies wholly outside every code section; the
        /// entry is dropped.
        ...
        /// The `LC_SYMTAB` symbol or string table region is outside the slice;
        /// symbols are unavailable.
        case symbolTableOutOfBounds
        /// An `nlist_64` entry's name offset is outside the string table; the
        /// symbol is dropped.
        ...
        /// The `LC_FUNCTION_STARTS` ULEB128 stream is malformed (truncated
        /// mid-value or a value exceeding 64 bits); addresses decoded before
        /// the malformation are kept.
