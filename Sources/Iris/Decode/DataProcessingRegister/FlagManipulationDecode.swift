...
enum FlagManipulationDecode {
    /// Decode an RMIF / SETF8 / SETF16 encoding; returns `nil` when the word
    /// is not one of them (so the add/subtract-with-carry decoder handles the
    /// rest of the `bits[23:21]=000` tier).
    @inline(__always)
    @_optimize(speed)
