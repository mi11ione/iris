...
extension SVEFloatingPointDecode {
    ...
    @inline(__always)
    static func decodeUnpredicated3Op(_ e: UInt32, _ a: UInt64) -> DecodedDraft {
        let sz = (e >> 22) & 0b11
        let opc = (e >> 10) & 0b111
    ...
    @inline(__always)
    static func decodeReciprocalEstimate(_ e: UInt32, _ a: UInt64) -> DecodedDraft {
        ...
        return DecodedDraft(
            ...
            semanticWrites: vecMask(d), category: .sve,
            operands: [vec(d, size), vec(n, size)],
            scalableEffect: .readsStreamingMode,
        )
    }
    ...
    @inline(__always)
    static func decodeUnaryTrigCarveOut(_ e: UInt32, _ a: UInt64) -> DecodedDraft {
        if (e & 0xFF2E_E000) == 0x040C_A000 {
            return decodeFPAbsNeg(e, a)
        }
        if (e & 0xFF20_FC00) == 0x0420_B000 {
            return decodeFTSSEL(e, a)
        }
        return decodeFEXPA(e, a) // scope guarantees the FEXPA signature here
    }

    @inline(__always)
    static func decodeFPAbsNeg(_ e: UInt32, _ a: UInt64) -> DecodedDraft {
        guard let size = fpSize(e) else { return undefined(e, a) }
        let mnemonic: Mnemonic = (e >> 16) & 1 == 0 ? .fabs : .fneg
        let merging = (e >> 20) & 1 == 1
        return unaryDraft(
            e, a, mnemonic, size, size,
            merging ? .merging : .zeroing, partial: merging,
        )
    }

    @inline(__always)
    static func decodeFTSSEL(_ e: UInt32, _ a: UInt64) -> DecodedDraft {
        guard let size = fpSize(e) else { return undefined(e, a) }
        let d = zd(e), n = zn(e), m = zm(e)
        return DecodedDraft(
    ...
    @inline(__always)
    static func decodeFEXPA(_ e: UInt32, _ a: UInt64) -> DecodedDraft {
        guard let size = fpSize(e) else { return undefined(e, a) }
        let d = zd(e), n = zn(e)
        return DecodedDraft(
